import { readInput, getGridString, getAdjacent, type Grid, type Pos, getAdjacentPos } from "@utils/helpers.ts"
...
function removeDFS(pos: Pos, grid: Grid<string>, withRecursion: boolean): number {
    ...
    if (
        row !== undefined &&
        row[pos.col] === "@" &&
        (sum(getAdjacent(pos, grid)
            .map((x) => x === "@" ? 1 : 0)) < 4)
    ) {
        row[pos.col] = "."
        if (withRecursion) {
            return (
                1 + sum(getAdjacentPos(pos, grid).map(
                    (pos) => removeDFS(pos, grid, withRecursion))
                )
            )
        } else {
            return 1
